 ...
 * All rights reserved.
 *
 * Date: Tue May 7 11:39:18 2013 +0800
 *
 ***
 *
...
var textUnderline = function(context,text,x,y,color,textSize,align){
    ...
    var startX = 0;
    var startY = y+(parseInt(textSize)/15) + 2;
    var endX = 0;
    var endY = startY;
		...
		if (matrix) {
            if(this.path.data.type === 'circle') {
                ...
                if (change) {
                    point._x = x;
                    point._y = y;
                    /*if(this.path.roughBounds.width < 16) {
                        i  = 2;
                        if (handleIn) {
...
var Path = this.Path = PathItem.extend({
	...
	_applyMatrix: function(matrix) {
        ...
        if(this.data.type === 'circle') {
            var centerX = (this._segments[0].point.x + this._segments[2].point.x)/2;
            var centerY = (this._segments[0].point.y + this._segments[2].point.y)/2;
            this._segments[0].point.x = centerX - 4;
            this._segments[0].point.y = centerY;

            this._segments[1].point.x = centerX;
            this._segments[1].point.y = centerY - 4;

            this._segments[2].point.x = centerX + 4;
            this._segments[2].point.y = centerY;

            this._segments[3].point.x = centerX;
            this._segments[3].point.y = centerY + 4;
        }
        if(this.data.type === 'bordercross1' || this.data.type === 'bordercross2') {
            var lLine = (this._segments[0]._point.y - this._segments[1]._point.y)/(this._segments[0]._point.x - this._segments[1]._point.x);
            var crossCenter1X = (this._segments[0]._point.x + this._segments[1]._point.x)/2;
            var crossCenter1Y = (this._segments[0]._point.y + this._segments[1]._point.y)/2;
            this._segments[0].point.x = crossCenter1X + (5/Math.sqrt(1 + lLine * lLine));
            this._segments[0].point.y = crossCenter1Y + (5 * lLine/Math.sqrt(1 + lLine * lLine));

            this._segments[1].point.x = crossCenter1X - (5/Math.sqrt(1 + lLine * lLine));
            this._segments[1].point.y = crossCenter1Y - (5 * lLine/Math.sqrt(1 + lLine * lLine));
			...
			if (style.getFillColor())
            {
                if(this.data.type === 'text') {
                    if(this.bounds.height > 24) {
                        ctx.scale(24./this.bounds.height, 24./this.bounds.height);
                    }
                ...
                if(style.getFontUnderline() !== "") {
                    textUnderline(ctx, line, 0, 0, this._style.fillColor._canvasStyle, style.getFontStyleSize(), 'left');
                }
            }
			if (style.getStrokeColor())
...
var FillImage = this.FillImage = Base.extend(new function() {
    ...
    initialize: function(image, rotation) {
        this.image = image;
...
var Style = this.Style = Base.extend(new function() {
	...
	getFontStyle: function() {
		...
		return this.getBold() + ' ' + this.getItalic() + ' ' + (/[a-z]/i.test(size) ? size + ' ' : size + 'px ')
				+ this.getFont();
	}
});
